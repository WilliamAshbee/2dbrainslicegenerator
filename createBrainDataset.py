...
def sliceSurface(sRep,whre,smaxs,smins,mrange,mmins):
...
def slice(id,mRep,sRep,whre,mmaxs,mmins,smaxs,smins):
    img = sliceMri(mRep,whre,mmaxs,mmins)
    np.save('./slices/'+id+'.img.npy',img)
    mmins, mmaxs = bbox1_2d(img)
    points = sliceSurface(sRep,whre,smaxs,smins,mmaxs-mmins,mmins)
    np.save('./slices/'+id+'.points.npy',points)
    
    #points[:,1] = points[:,1]+100.0
    #points[:,0] = points[:,0]+125.0
    fig = plt.figure()
    plt.imshow(img)
    plt.scatter(points[:,0],points[:,1],c="red", s= .01)
    #plt.xlim(0, 256)
    #plt.ylim(0, 256)
    ...
    while True:
        ...
        try:
            ...
            if mri != None and surface !=None:
                ...
                slice(id4,mri,surface,.5,mmaxs,mmins,smaxs,smins)
